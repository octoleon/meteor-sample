...
class ShipAddressBook extends Component {
  static propTypes = {
  ...
  /**
   * @method onSelect
   * @summary updating an address if it's been selected as the default shipping address.
   * @since 2.0.0
   * @param {String} _id - address object _id.
   * @param {String} usage - the address usage "shipping".
   * @ignore
   */
  onSelect = (_id, usage) => {
  ...
  renderContent() {
    ...
    if (mode === "entry") {
      content = (
        <Components.ShipAddressBookForm
          add={this.onAdd}
      ...
      content = (
        <Components.ShipAddressBookGrid
...
registerComponent("ShipAddressBook", ShipAddressBook);

export default ShipAddressBook;
