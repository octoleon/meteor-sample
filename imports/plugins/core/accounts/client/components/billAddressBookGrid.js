...
class BillAddressBookGrid extends Component {
  static propTypes = {
    ...
    // selects a default billing address
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // check to see if the new address is now the default billing address
   ...
   * @summary updating the compoenet state with the new default billing address.
   * @since 2.0.0
   ...
   * a default billing address. This handler sets the default address
  ...
  renderAddressGrid() {
    ...
    return addressBook.map((address) => {
      const { defaultBillingAddressId } = this.state;
...
registerComponent("BillAddressBookGrid", BillAddressBookGrid);

export default BillAddressBookGrid;
