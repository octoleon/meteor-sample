...
class BillAddressBook extends Component {
  static propTypes = {
  ...
  /**
   * @method onSelect
   * @summary updating an address if it's been selected as the default billing address.
   * @since 2.0.0
   * @param {String} _id - address object _id.
   * @param {String} usage - the address usage "billing".
   * @ignore
   */
  onSelect = (_id, usage) => {
   ...
   */
  renderCheckoutIcon() {
    const { icon, position } = this.props.heading.checkout;
  ...
  renderHeading() {
    ...
    return (
      <div className="panel-heading-billing">
  ...
  renderControlBar() {
    ...
    if (mode === "entry" && !this.hasEditAddress) {
      ...
      controlBarContent = (
        <button className="rui btn btn-primary solid btn-block login-button single-login-button continue-guest" onClick={this.setEntryMode}>
  ...
  renderContent() {
    ...
    if (mode === "entry") {
      content = (
        <Components.BillAddressBookForm
          add={this.onAdd}
      ...
      content = (
        <Components.BillAddressBookGrid
  ...
  render() {
    return (
      <div className="panel panel-default panel-address-book-billing">
        {this.renderHeading()}
...
registerComponent("BillAddressBook", BillAddressBook);

export default BillAddressBook;
