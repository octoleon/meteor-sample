...
export const methods = {
  ...
  "orders/sendNotification"(order, action) {
    ...
    if (order) {
      ...
      const dataForEmail = {
        ...
        orderUrl: `account/profile#orders#${order._id}`,
        shipping: {
          tracking,
          carrier,
      ...
      if (order.workflow.status === 'new') {
        tpl = `orders/new-admin`;
        subject = `orders/new-admin/subject`;
  ...
  /**
   * @name orders/updateOrdersOfGuest
   * @method
   * @memberof Orders/Methods
   * @summary Update userId on Guest user's all orders once Guest user sign up
   * @param {String} email - valid email address
   * @return {String} returns orders update result
   */
  "orders/updateOrdersOfGuest"(email) {
    check(email, String);
    /**
     *Update userId on Guest user's all orders once Guest user sign up
     *because his/her previous orders include guest user ID. Once sing up, guest user will be no anonymous, and got new userID.
     */
    ...
    Orders.update({ email }, { $set: { userId } }, { multi: true });
  },
